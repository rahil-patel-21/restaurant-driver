...
Future<void> updateStatus() async {
  try {
    final String url =
        '${GlobalConfiguration().getString('api_base_url')}availability/${currentUser.value.id}?available=${currentUser.value.isAvailable ? 0 : 1}&api_token=${currentUser.value.apiToken}';
    final client = new http.Client();
    ...
    if (body != null) {
      print(body);
      if (body['data'] != null) {
        if (body['data']['available'] != null) {
          print(body['data']['available']);
          currentUser.value.isAvailable = body['data']['available'];
          SharedPreferences prefs = await SharedPreferences.getInstance();
          await prefs.setBool('available', body['data']['available']);
        }
      }
    }
  } catch (error) {
...
Future<void> logout() async {
  ...
  await prefs.remove('available');
}

void setCurrentUser(jsonString, {bool updatingStatus = false}) async {
  try {
    if (json.decode(jsonString)['data'] != null) {
      ...
      await prefs.setBool(
          'available', json.decode(jsonString)['data']['driver']['available']);
    }
  } catch (e) {
...
Future<User> getCurrentUser() async {
  ...
  if (currentUser.value.auth == null && prefs.containsKey('current_user')) {
    ...
    currentUser.value.isAvailable = await prefs.get('available') ?? true;
